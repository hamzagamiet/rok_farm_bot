...
def complete_verification(window, window_key):
    ...
    TEMPLATE_LIST_DIR = os.path.join(
        BASE_DIR, "static", f"verification_template_list_{window_key}.jpg"
    )
    TEMPLATE_DIR = os.path.join(BASE_DIR, "static", f"template_{window_key}.jpg")
    TEMPLATE_ROTATED_DIR = os.path.join(
        BASE_DIR, "static", f"template_rotate_{window_key}.jpg"
    )
    image = Image.open(SOURCE_DIR)
    source = image.crop(
        (
            loc["verification_source"]["x0"],
            loc["verification_source"]["y0"],
            loc["verification_source"]["x1"],
            loc["verification_source"]["y1"],
        )
    )
    source.save(SOURCE_DIR)
    template_list = image.crop(
        (
            loc["verification_temp"]["x0"],
            loc["verification_temp"]["y0"],
            loc["verification_temp"]["x1"],
            loc["verification_temp"]["y1"],
        )
    )
    ...
    for x in range(5):
        ...
        ret, thresh_template = cv2.threshold(
            template_image, 127, 255, cv2.THRESH_BINARY_INV
        )
